...
const SERVICES = [
  { icon: "🏦", title: "Private Banking", desc: "Account opening, wealth advisory introductions, family office coordination across Italian and Swiss institutions" },
  ...
  { icon: "🏠", title: "Real Estate", desc: "Property search in Milan, Lake Como & Tuscany. Due diligence, negotiation, and renovation management" },
  { icon: "🏥", title: "Healthcare", desc: "Private English-speaking GP, VIP access to San Raffaele, Humanitas, IEO. Home care coordination" },
  { icon: "🎓", title: "Education", desc: "International school placement: ASM, ISM, St. Louis, Sir James Henderson. University guidance" },
  { icon: "🛂", title: "Immigration", desc: "Residence permits, golden visa, citizenship pathway, corporate setup. Full bureaucratic fast-track" },
];
...
function RetireeSection({ setPage }) {
  const items = [
    { icon:"🏥", title:"Healthcare, Handled", desc:"English-speaking private GP on retainer, fast-track specialist access at San Raffaele and Humanitas, home care coordination, and medical concierge for visiting family." },
    { icon:"📋", title:"Succession Planning", desc:"Cross-border inheritance structuring for families spread across 2-3 countries. We coordinate between your Italian, UK, Swiss, and US advisors." },
    { icon:"👨‍🍳", title:"Domestic Staff", desc:"Vetted, qualified staff — custodian, cook, driver, gardener. And a dedicated house manager to coordinate them all. Employment contracts, payroll, everything handled." },
    { icon:"🔐", title:"Property & Security", desc:"Maintenance, security systems, and property management when you are away. Your villa is looked after year-round." },
    { icon:"🤝", title:"Community & Social", desc:"Introductions to golf clubs, yacht clubs, cultural associations, and the international community. Loneliness is the real enemy — we make sure it never visits." },
    { icon:"🚁", title:"Transport & Logistics", desc:"Helicopter transfers Como-Milan, jet card setup, airport VIP services, chauffeur arrangements. Move seamlessly between your Italian life and the world." },
  ];
  return (
...
function CostOfLiving() {
  ...
  const rows = [
    ["🏠 Villa / Apartment", c.villa, m.villa],
    ["👨‍🍳 Domestic Staff", c.staff, m.staff],
    ["🏥 Healthcare (family)", c.health, m.health],
    ["🍷 Dining & Lifestyle", c.dining, m.dining],
    ["🎓 International School", c.school, m.school],
    ["💰 Tax (foreign income)", cTax, m.tax],
  ];
      ...
      <FadeIn delay={100}>
        ...
        <div style={{ overflowX:"auto" }}>
          <table style={{ width:"100%",borderCollapse:"collapse" }}>
            <thead><tr>
              <th style={{ textAlign:"left",padding:"14px 16px",borderBottom:"2px solid "+C.gold,color:C.textDim,fontSize:12,letterSpacing:1 }}>EXPENSE</th>
              <th style={{ textAlign:"right",padding:"14px 16px",borderBottom:"2px solid "+C.gold,color:C.textDim,fontSize:12,letterSpacing:1 }}>{c.flag} {c.name.toUpperCase()}</th>
              <th style={{ textAlign:"right",padding:"14px 16px",borderBottom:"2px solid "+C.gold,color:C.gold,fontSize:12,letterSpacing:1 }}>🇮🇹 MILAN</th>
              <th style={{ textAlign:"right",padding:"14px 16px",borderBottom:"2px solid "+C.gold,color:C.textDim,fontSize:12,letterSpacing:1 }}>SAVING</th>
            </tr></thead>
            <tbody>
              {rows.map(([label,cv,mv],i) => <tr key={i}>
                <td style={{ padding:"14px 16px",borderBottom:"1px solid "+C.border,color:C.text,fontSize:14 }}>{label}</td>
                <td style={{ padding:"14px 16px",borderBottom:"1px solid "+C.border,color:"#ef4444",fontSize:14,textAlign:"right" }}>{fmt(cv)}</td>
                <td style={{ padding:"14px 16px",borderBottom:"1px solid "+C.border,color:C.gold,fontSize:14,textAlign:"right" }}>{fmt(mv)}</td>
                <td style={{ padding:"14px 16px",borderBottom:"1px solid "+C.border,color:cv-mv>0?"#22c55e":"#ef4444",fontSize:14,textAlign:"right",fontWeight:600 }}>{cv-mv>0?"+":""}{fmt(cv-mv)}</td>
              </tr>)}
              <tr>
                <td style={{ padding:"16px",borderTop:"2px solid "+C.gold,color:C.white,fontSize:15,fontWeight:600 }}>TOTAL</td>
                <td style={{ padding:"16px",borderTop:"2px solid "+C.gold,color:"#ef4444",fontSize:18,textAlign:"right",fontWeight:600,fontFamily:"Georgia,serif" }}>{fmt(cTotal)}</td>
                <td style={{ padding:"16px",borderTop:"2px solid "+C.gold,color:C.gold,fontSize:18,textAlign:"right",fontWeight:600,fontFamily:"Georgia,serif" }}>{fmt(mTotal)}</td>
                <td style={{ padding:"16px",borderTop:"2px solid "+C.gold,color:saving>0?"#22c55e":"#ef4444",fontSize:18,textAlign:"right",fontWeight:600,fontFamily:"Georgia,serif" }}>{saving>0?"+":""}{fmt(saving)}</td>
              </tr>
            </tbody>
          </table>
        </div>
    ...
    <section style={{ padding:"120px 24px 80px",maxWidth:1200,margin:"0 auto" }}>
      ...
      <div style={{ display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(280px,1fr))",gap:24 }}>
        {PROPERTIES.map((p,i) => <FadeIn key={p.id} delay={i*60}><div style={{ background:C.card,border:"1px solid "+C.border,overflow:"hidden",transition:"all 0.3s" }} onMouseEnter={e => e.currentTarget.style.borderColor=C.goldDim} onMouseLeave={e => e.currentTarget.style.borderColor=C.border}>
          <div style={{ height:200,background:"linear-gradient("+p.grad+")",position:"relative",display:"flex",alignItems:"center",justifyContent:"center" }}><span style={{ fontFamily:"Georgia,serif",fontSize:48,color:"rgba(255,255,255,0.15)" }}>🏡</span><div style={{ position:"absolute",top:12,left:12,background:"rgba(201,169,110,0.9)",color:C.bg,padding:"4px 12px",fontSize:11,fontWeight:600,letterSpacing:1 }}>{p.tag}</div></div>
  ...
  return (
    <section id="contact" style={{ padding:"100px 24px",background:C.card }}>
      <div style={{ maxWidth:600,margin:"0 auto" }}>
        ...
        {sent ? <FadeIn><div style={{ textAlign:"center",padding:48,background:C.bg,border:"1px solid "+C.gold }}><div style={{ fontSize:48,marginBottom:16 }}>✓</div><h3 style={{ fontFamily:"Georgia,serif",fontSize:24,color:C.white,marginBottom:8 }}>Thank you</h3><p style={{ color:C.textDim }}>We will be in touch within 24 hours.</p></div></FadeIn> :
